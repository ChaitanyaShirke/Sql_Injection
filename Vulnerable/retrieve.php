...
if (isset($_POST['pname']) && isset($_POST['patient_id'])) {
  $name = trim($_POST['pname']);
  $patient_id = trim($_POST['patient_id']);
  


  if (empty($name) || empty($patient_id) || !ctype_digit($patient_id)) {
    echo "<p style='color:red; font-weight:bold;'>Invalid credentials. Please provide valid Patient name and Patient ID.</p>";
  } else {
    $query = "SELECT * FROM vpatientsrecord WHERE Name = '$name' AND patient_id = '$patient_id'";
